...
Evaluates on the held-out unseen Chunk 6:
...
6. Advanced Trained MoE (Linear-3)
7. Advanced Trained MoE (Linear-5)
8. Advanced Unified Speculative Engine (Adaptive Tree-MoE)
"""
...
def generate_advanced_adaptive_tree(model, curr_e, curr_h, curr_r):
    draft_count = 1
    paths = []
    ...
    # Accurate router transition (now trained to 68.6% overlap!)
    r1 = model.transition_router(h1, curr_r, beta=0.5)
    ...
    # Sub-branch at Depth 2 if alternative is viable
    if p2_2 >= 0.15:
    ...
    # Root branch if first token was ambiguous
...
def run_master_benchmark():
    ...
    print("=========================================================================================")
    print(" FINAL ADVANCED BENCHMARK: ADVANCED MoE vs FIRST HYBRID MoE vs DENSE BASELINE")
    print(" (Evaluated on 100% Held-Out Unseen Test Chunk 006)")
    print("=========================================================================================\n")

    dense_model = DenseMTPDrafter()
    dense_model.load_state_dict(torch.load(MODELS_DIR / "dense_mtp.pt", map_location="cpu"))
    dense_model.eval()

    first_moe_model = HybridMoEMTPDrafter()
    first_moe_model.load_state_dict(torch.load(MODELS_DIR / "hybrid_moe_mtp.pt", map_location="cpu"), strict=False)
    first_moe_model.eval()

    adv_model = HybridMoEMTPDrafter()
    adv_model.load_state_dict(torch.load(MODELS_DIR / "hybrid_moe_mtp_advanced.pt", map_location="cpu"), strict=False)
    adv_model.eval()

    # Load chunk 6 (held-out test set)
    d = torch.load(MTP_FEATURES_DIR / "mtp_chunk_006.pt", map_location="cpu")
    ...
    competitors = [
        ...
        "4. First Hybrid MoE (Linear-5)",
        "5. First Hybrid MoE (Tree-7b Fixed)",
        "6. Advanced MoE (Linear-3 Dynamic)",
        "7. Advanced MoE (Linear-5 Dynamic)",
        "8. Advanced Engine (Adaptive Tree)",
    ]
    ...
    with torch.no_grad():
        for seq_idx in range(len(test_e)):
            ...
            for t in range(0, seq_len - 6, 4):
                ...
                stats["5. First Hybrid MoE (Tree-7b Fixed)"]["drafted_tokens"] += d5
                stats["5. First Hybrid MoE (Tree-7b Fixed)"]["accepted_tokens"] += acc5
                stats["5. First Hybrid MoE (Tree-7b Fixed)"]["yield_sum"] += (1 + acc5)
                stats["5. First Hybrid MoE (Tree-7b Fixed)"]["by_dom_yield"][dom] += (1 + acc5)
                stats["5. First Hybrid MoE (Tree-7b Fixed)"]["by_dom_evals"][dom] += 1
                stats["5. First Hybrid MoE (Tree-7b Fixed)"]["evals"] += 1

                # 6. Advanced MoE Linear-3 Dynamic
                p6, d6 = generate_linear(adv_model, True, curr_e, curr_h, curr_r, 3, dynamic=True)
                acc6 = verify_tree_paths(p6, gt)
                stats["6. Advanced MoE (Linear-3 Dynamic)"]["drafted_tokens"] += d6
                stats["6. Advanced MoE (Linear-3 Dynamic)"]["accepted_tokens"] += acc6
                stats["6. Advanced MoE (Linear-3 Dynamic)"]["yield_sum"] += (1 + acc6)
                stats["6. Advanced MoE (Linear-3 Dynamic)"]["by_dom_yield"][dom] += (1 + acc6)
                stats["6. Advanced MoE (Linear-3 Dynamic)"]["by_dom_evals"][dom] += 1
                stats["6. Advanced MoE (Linear-3 Dynamic)"]["evals"] += 1

                # 7. Advanced MoE Linear-5 Dynamic
                p7, d7 = generate_linear(adv_model, True, curr_e, curr_h, curr_r, 5, dynamic=True)
                acc7 = verify_tree_paths(p7, gt)
                stats["7. Advanced MoE (Linear-5 Dynamic)"]["drafted_tokens"] += d7
                stats["7. Advanced MoE (Linear-5 Dynamic)"]["accepted_tokens"] += acc7
                stats["7. Advanced MoE (Linear-5 Dynamic)"]["yield_sum"] += (1 + acc7)
                stats["7. Advanced MoE (Linear-5 Dynamic)"]["by_dom_yield"][dom] += (1 + acc7)
                stats["7. Advanced MoE (Linear-5 Dynamic)"]["by_dom_evals"][dom] += 1
                stats["7. Advanced MoE (Linear-5 Dynamic)"]["evals"] += 1

                # 8. Advanced Engine (Adaptive Tree)
                p8, d8 = generate_advanced_adaptive_tree(adv_model, curr_e, curr_h, curr_r)
                acc8 = verify_tree_paths(p8, gt)
                stats["8. Advanced Engine (Adaptive Tree)"]["drafted_tokens"] += d8
                stats["8. Advanced Engine (Adaptive Tree)"]["accepted_tokens"] += acc8
                stats["8. Advanced Engine (Adaptive Tree)"]["yield_sum"] += (1 + acc8)
                stats["8. Advanced Engine (Adaptive Tree)"]["by_dom_yield"][dom] += (1 + acc8)
                stats["8. Advanced Engine (Adaptive Tree)"]["by_dom_evals"][dom] += 1
                stats["8. Advanced Engine (Adaptive Tree)"]["evals"] += 1

    print(f"[EVAL] Master benchmark completed in {time.time() - t0:.1f}s.\n")
    ...
    print("--- [1] FINAL MASTER SPECULATIVE PERFORMANCE MATRIX ---")
